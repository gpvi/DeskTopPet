...
- `npm run build` 当前仅剩 `lottie-web` 的 eval 非阻塞告警，不影响功能验收结论。
...
- 已完成分包优化：构建侧采用 `manualChunks` + Lottie lazy 加载，chunk size 告警已消除。
- 当前构建告警仅剩 `lottie-web` 的 eval 非阻塞项；Smoke 最新仍为 `pass=26,warn=0,fail=0`。
